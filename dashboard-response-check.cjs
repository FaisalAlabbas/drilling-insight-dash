...
async function runCheck(checkNumber) {
  ...
  try {
    ...
    if (!response || !response.ok()) {
      ...
      await Promise.all([
        page.locator('text=Surface Dashboard').first().waitFor({ state: 'visible', timeout: TIMEOUT_MS }),
        page.locator('text=Telemetry').first().waitFor({ state: 'visible', timeout: TIMEOUT_MS }),
        page.locator('text=AI Recommendation').first().waitFor({ state: 'visible', timeout: TIMEOUT_MS }),
        page.locator('text=Recent Decisions').first().waitFor({ state: 'visible', timeout: TIMEOUT_MS }),
      ]);
  ...
  const screenshotPath = status === 'FAIL'
    ? `response-failure-check-${checkNumber}.png`
    : null;
  ...
  const record = {
    ...
    withinTarget: totalResponseTimeMs !== null ? totalResponseTimeMs <= RESPONSE_TARGET_MS : false,
    reason,
