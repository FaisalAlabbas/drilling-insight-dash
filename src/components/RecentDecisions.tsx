...
export function RecentDecisions() {
  ...
  return (
    <div className="bg-card border border-border rounded-lg p-4">
      <h3 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-3">Recent Decisions</h3>
      <div className="flex gap-2 overflow-x-auto pb-2">
        {recent.map((d, i) => {
          const conf = d.confidence_score;
          const confColor = conf >= 0.75 ? 'border-signal-green/40' : conf >= 0.5 ? 'border-signal-yellow/40' : 'border-signal-red/40';
          return (
            <button
              key={i}
              onClick={() => { setSelectedDecision(d); setDrawerOpen(true); }}
              className={cn(
            ...
            >
              ...
              <p className={cn("text-[10px] font-mono",
                conf >= 0.75 ? 'text-signal-green' : conf >= 0.5 ? 'text-signal-yellow' : 'text-signal-red'
              )}>
                {(conf * 100).toFixed(0)}%
              </p>
