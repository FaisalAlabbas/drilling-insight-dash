...
export function ReportingView() {
  ...
  const [reportType, setReportType] = useState<'performance' | 'safety' | 'operations'>('performance');

  const stats = useMemo(() => {
    const totalDecisions = decisions.length;
    const acceptedDecisions = decisions.filter(d => d.gate_outcome === 'ACCEPTED').length;
    ...
    const commandCounts: Record<SteeringCommand, number> = {} as any;
    decisions.forEach(d => {
  ...
  const confidenceChart = useMemo(() => {
    ...
    const recentDecisions = decisions.filter(d => new Date(d.timestamp).getTime() > hourAgo);
    ...
    const grouped = Array.from({ length: 12 }, (_, i) => {
      ...
      return {
        ...
        avgConfidence: interval.length > 0
          ? (interval.reduce((sum, d) => sum + d.confidence_score, 0) / interval.length * 100)
          : 0,
    ...
    <div className="space-y-4">
      ...
      {reportType === 'performance' && (
        <div className="space-y-4">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            <div className="bg-card border border-border rounded-lg p-3">
              <p className="text-[10px] text-muted-foreground uppercase">Acceptance Rate</p>
              <p className="text-xl font-bold mt-1 text-signal-green">{stats.acceptanceRate.toFixed(1)}%</p>
            </div>
            <div className="bg-card border border-border rounded-lg p-3">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <div className="bg-card border border-border rounded-lg p-4">
              <h3 className="text-xs font-semibold uppercase tracking-wider mb-3">Confidence Trend (Last Hour)</h3>
              <div className="h-[200px]">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={confidenceChart}>
                    ...
                    <Tooltip contentStyle={{ backgroundColor: 'hsl(220, 18%, 12%)', border: 'none', borderRadius: '6px' }} />
                    <Line type="monotone" dataKey="avgConfidence" stroke="hsl(210, 20%, 92%)" strokeWidth={2} dot={false} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </div>
            ...
            <div className="bg-card border border-border rounded-lg p-4">
              <h3 className="text-xs font-semibold uppercase tracking-wider mb-3">Command Distribution</h3>
              <div className="h-[200px]">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={commandChart}>
                    <CartesianGrid strokeDasharray="3 3" stroke="hsl(220, 14%, 18%)" />
                    <XAxis dataKey="command" tick={{ fontSize: 9 }} angle={-45} textAnchor="end" height={80} />
                    <YAxis tick={{ fontSize: 10 }} />
                    <Tooltip contentStyle={{ backgroundColor: 'hsl(220, 18%, 12%)', border: 'none', borderRadius: '6px' }} />
                    <Bar dataKey="count" fill="hsl(210, 20%, 92%)" radius={[4, 4, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </div>
      ...
      {reportType === 'safety' && (
        <div className="space-y-4">
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
            <div className="bg-card border border-border rounded-lg p-3">
              <p className="text-[10px] text-muted-foreground uppercase">Critical Alerts</p>
              <p className="text-xl font-bold mt-1 text-signal-red">{stats.criticalAlerts}</p>
            </div>
            <div className="bg-card border border-border rounded-lg p-3">
              <p className="text-[10px] text-muted-foreground uppercase">Warnings</p>
              <p className="text-xl font-bold mt-1 text-signal-yellow">{stats.warningAlerts}</p>
            </div>
            <div className="bg-card border border-border rounded-lg p-3">
              <p className="text-[10px] text-muted-foreground uppercase">Total Alerts</p>
              <p className="text-xl font-bold mt-1">{stats.criticalAlerts + stats.warningAlerts}</p>
            </div>
          </div>

          <div className="bg-card border border-border rounded-lg p-4">
            <h3 className="text-xs font-semibold uppercase tracking-wider mb-3">Rejection Reasons</h3>
      ...
      {reportType === 'operations' && (
        <div className="space-y-4">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            <div className="bg-card border border-border rounded-lg p-3">
              <p className="text-[10px] text-muted-foreground uppercase">Telemetry Points</p>
              <p className="text-xl font-bold mt-1">{telemetry.length}</p>
            </div>
            <div className="bg-card border border-border rounded-lg p-3">
              <p className="text-[10px] text-muted-foreground uppercase">Model Decisions</p>
              <p className="text-xl font-bold mt-1">{stats.totalDecisions}</p>
            </div>
            <div className="bg-card border border-border rounded-lg p-3">
          ...
          <div className="bg-card border border-border rounded-lg p-4">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <div>
                <p className="text-xs font-medium text-muted-foreground mb-2">Processing Duration</p>
