...
export function DetailDrawer() {
  const { drawerOpen, setDrawerOpen, selectedDecision, selectedAlert, telemetry } = useDashboard();
  ...
  return (
    <>
      <div className="fixed inset-0 bg-background/60 z-40" onClick={() => setDrawerOpen(false)} />
      <div className="fixed right-0 top-0 h-full w-full max-w-md bg-card border-l border-border z-50 flex flex-col overflow-hidden animate-in slide-in-from-right duration-200">
        <div className="flex items-center justify-between p-4 border-b border-border shrink-0">
          ...
          <button onClick={() => setDrawerOpen(false)} className="p-1 hover:bg-muted rounded">
            <X className="h-4 w-4" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {decision && (
            <>
              <div className="space-y-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className={cn(
                    ...
                    decision.gate_outcome === 'ACCEPTED' ? 'bg-signal-green/15 text-signal-green' : 'bg-signal-red/15 text-signal-red'
                  )}>
                ...
                <div className="grid grid-cols-2 gap-2 text-xs">
                  <div className="bg-muted rounded-md p-2">
                    ...
                    <p className="font-mono font-semibold">{(decision.confidence_score * 100).toFixed(1)}%</p>
                  </div>
                  <div className="bg-muted rounded-md p-2">
                  ...
                  {decision.rejection_reason && (
                    <div className="bg-muted rounded-md p-2">
                      ...
                      <p className="font-mono text-signal-red">{decision.rejection_reason}</p>
                    </div>
                  )}
                  {decision.fallback_mode && (
                    <div className="bg-muted rounded-md p-2">
                      <p className="text-muted-foreground">Fallback</p>
                      <p className="font-mono text-signal-amber">{decision.fallback_mode}</p>
                    </div>
                  )}
                </div>
              ...
              <div className="border-t border-border pt-4">
                <h4 className="text-xs font-semibold text-muted-foreground mb-2 uppercase tracking-wider">Feature Summary</h4>
                ...
                {alert.related_signals.length > 0 && (
                  <div className="flex gap-1 flex-wrap">
                    {alert.related_signals.map(s => (
                      <span key={s} className="text-[10px] px-2 py-1 bg-muted rounded-md text-muted-foreground font-mono">{s}</span>
                    ))}
                  </div>
                )}
              </div>
          ...
          {/* Linked Telemetry */}
          <div className="border-t border-border pt-4">
            <h4 className="text-xs font-semibold text-muted-foreground mb-2 uppercase tracking-wider">Linked Telemetry</h4>
            <div className="h-[150px]">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={contextData}>
                  ...
                  />
                  <Line type="monotone" dataKey="vibration" stroke="hsl(0, 72%, 55%)" strokeWidth={1} dot={false} />
                  <Line type="monotone" dataKey="wob" stroke="hsl(187, 85%, 53%)" strokeWidth={1} dot={false} />
                </LineChart>
              </ResponsiveContainer>
            </div>
          </div>
