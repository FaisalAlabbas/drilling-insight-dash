...
const SIGNALS = [
  {
    ...
    unit: "klbf",
    color: "hsl(187, 85%, 53%)",
    limit: OPERATING_LIMITS.wob_range[1],
  },
  {
    key: "torque_kftlb",
    label: "Torque",
    unit: "kft·lb",
    color: "hsl(142, 70%, 49%)",
    limit: OPERATING_LIMITS.torque_range[1],
  },
  {
    key: "rpm",
    label: "RPM",
    unit: "rpm",
    color: "hsl(45, 93%, 58%)",
    limit: OPERATING_LIMITS.rpm_range[1],
  },
  {
    key: "vibration_g",
    label: "Vibration",
    unit: "g",
    color: "hsl(0, 72%, 55%)",
    limit: OPERATING_LIMITS.max_vibration_g,
  },
  {
    key: "inclination_deg",
    label: "Inclination",
  ...
  { key: "rop_ft_hr", label: "ROP", unit: "ft/hr", color: "hsl(32, 95%, 55%)" },
  {
    ...
    unit: "°/100ft",
    color: "hsl(330, 70%, 55%)",
    limit: OPERATING_LIMITS.max_dls_deg_100ft,
  },
...
export function TelemetryChart() {
  ...
  const chartData = useMemo(() => {
    ...
    return telemetry.slice(-sliceCount).map((p) => ({
      ...
      value: p[signal.key as keyof typeof p],
    }));
      ...
      <div className="flex gap-1 mb-3 flex-wrap">
        {SIGNALS.map((s, i) => (
          <button
            ...
            style={
              i === activeSignal
                ? { backgroundColor: `${s.color}20`, borderColor: `${s.color}40` }
                : {}
            }
          >
            {s.label}
